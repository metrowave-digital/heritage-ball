...
export async function POST(req: Request) {
  try {
    ...
    // 1️⃣ Add to Audience (auto-deduplicates by email)
    ...
    // 2️⃣ Send confirmation email (optional but recommended)
    await resend.emails.send({
      from: 'Heritage Ball <heritageball@imanational.org>',
      to: email,
      subject: 'You’re Locked In — Heritage Ball Vol. I',
      html: `
        <div style="background:#000;color:#fff;font-family:Arial,Helvetica,sans-serif;padding:40px 16px;">
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;margin:0 auto;border:1px solid rgba(255,255,255,0.15);">
    <tr>
      <td style="padding:40px;text-align:center;">
        ...
        <!-- Badge -->
        <div style="display:inline-block;padding:6px 14px;border:1px solid rgba(255,255,255,0.3);
          text-transform:uppercase;font-size:11px;letter-spacing:2px;margin-bottom:24px;">
          Heritage Ball • Vol. I
        </div>

        <!-- Title -->
        <h1 style="margin:0 0 16px;font-size:42px;font-weight:900;font-style:italic;letter-spacing:-1px;">
          Royal Bloodlines
        </h1>
        ...
        <!-- Event Info -->
        <table width="100%" style="margin:32px 0;border-top:1px solid rgba(255,255,255,0.2);border-bottom:1px solid rgba(255,255,255,0.2);">
          <tr>
            <td style="padding:16px;text-align:center;">
              <p style="margin:0;font-size:14px;letter-spacing:2px;text-transform:uppercase;color:#9ca3af;">
